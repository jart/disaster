...
;;
;; It works by creating a `.o` file using `make` (if you have a Makefile),
;; `cmake` (if you have a `CMakeLists.txt` file), the compile command in `compile_commands.json` (if said file exists and no `CmakeLists.txt` was found), or the default system
...
USE-COMPILE-DB: non NIL to use the compiler found in compile_commands.json (generated by CMake or Bear), NIL to use Make or default compiler options,
MAKE-ROOT: path to build root,
CWD: path to current source file,
REL-OBJ: path to object file (relative to project root),
OBJ-FILE: full path to object file (build root!)
  ...
  (let* ((file (or file (buffer-file-name)))
    ...
    (if (or (string-match-p disaster-c-regexp filename)
        ...
        (let* ((cwd (file-name-directory file)) ;; path to current source file
               ...
               (use-compile-db (file-exists-p (concat proj-root "compile_commands.json")))
          ...
          (when use-compile-db
            (let ((tmp (disaster-get-object-file-path-compile-db cc)))
              (when (file-exists-p (concat proj-root "CMakeLists.txt"))
